...
void drawLib(Method_Paint method)
{
   ...
   //if(dd == NULL)
    ...
    drawImage(dd, 300, 300, 0, 0, dd->width, dd->height, BOTTOM | RIGHT, 0.2f, 0.4f, 2, 0, REVERSE_HEIGHT);// REVERSE_HEIGHT를 하는 이유는 스크린샷은 화면이 반전되어 저장되기 때문임.

    SaveBitmapToFile((BYTE*)&dd, dd->potWidth, dd->potHeight, 24, 0, TEXT("C:\\Users\\user\\Documents\\GitHub\\Shooting\\aa.png"));
    setRGBA(1, 1, 1, 1);
